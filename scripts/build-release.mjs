...
function run(command, argumentsList) {
    ...
    if (result.error || result.status !== 0) {
        throw new Error(`${command} 执行失败：${result.error?.message || `退出码 ${result.status}`}`);
    }
}

const pkgBinary = path.join(rootDirectory, "node_modules", "@yao-pkg", "pkg", "lib-es5", "bin.js");

fs.rmSync(outputDirectory, { force: true, recursive: true });
fs.mkdirSync(outputDirectory, { recursive: true });
run(process.execPath, ["scripts/embed-camoufox.mjs"]);
run(process.execPath, ["scripts/embed-playwright-core.mjs"]);
run(process.execPath, [pkgBinary,
    "--target",
    "node22-win-x64",
    "--output",
    outputPath,
